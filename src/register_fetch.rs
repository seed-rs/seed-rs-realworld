...
use crate::{viewer, avatar, username, api, register_form};
...
pub fn register<Ms: 'static>(valid_form: &register_form::ValidForm, f: fn(Result<viewer::Viewer, Vec<register_form::Problem>>) -> Ms) -> impl Future<Item=Ms, Error=Ms>  {
...
fn process_fetch_object(fetch_object: fetch::FetchObject<String>) -> Result<viewer::Viewer, Vec<register_form::Problem>> {
    match fetch_object.result {
        Err(request_error) => {
            Err(vec![register_form::Problem::ServerError("Request error".into())])
        },
        Ok(response) => {
            if response.status.is_ok() {
                    ...
                    match viewer {
                        ...
                        Err(data_error) => {
                            Err(vec![register_form::Problem::ServerError("Data error".into())])
                        }
                    }
            } else {
                ...
                match error_messages {
                    Ok(error_messages) => {
                        let problems = error_messages
                            .into_iter()
                            .map(|message| {
                                register_form::Problem::ServerError(message)
                            }).collect();
                        Err(problems)
                    },
                    Err(data_error) => {
